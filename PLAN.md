...
| L4 | ✅ **PASSED on direct evidence, twice.** 2026-08-24: teardown left NOTHING — no apps, no resources, and the DNS record we wrote was removed, the name falling back to the domain's wildcard with the apex untouched. 2026-09-12: repeated on a Next.js site with a live custom domain, and confirmed from OUTSIDE the account — both the AWS address and the custom domain stopped resolving, and the record we had written was gone, so no name was left pointing at a deleted distribution. **`npm run certify` cannot corroborate this and its pass means nothing** — see the note below |
...
### `certify` cannot certify this poppy, and that is a platform bug

Run 2026-09-12, against an account with a live website standing: certify finished in **ten
seconds**, reported `footprintBefore: []`, and wrote `passed: true`. Its own warning says the
run was *"an empty no-op"*. Two innocent explanations were tested and both are wrong — the
Resource Groups Tagging index DOES cover Amplify (a real site returned its app and its branch
by tag), and the maintenance credentials DO carry `tag:GetResources`.

The cause: certify's blind-sweep guard treats a standing **CloudFormation stack** as its proof
that an empty sweep is suspicious, and the whole guard — including the tag-index warm-up —
only runs when a stack stands. **HostingPoppy creates no stacks.** It calls the Amplify API
directly, so the oracle legitimately does not exist and its absence reads as good news.

That is a hole in the platform's listing gate affecting every stackless poppy, not a defect
here. Specified for the platform in `agentspoppy/docs/specs/certify-stackless-blindness.md`.

**L4 is therefore listed on the evidence above, not on a certificate.** Do not cite
`leaves-no-trace.cert.json`: it is a vacuous pass, and it carries the AWS account id it ran
against (hence `*.cert.json` in `.gitignore`). When the platform fix lands, re-run certify
against a site that is actually standing and record the real result here.
...
| L4 | Done — see §4. Listed on direct evidence, not on `certify` |
