...
**`certify` cannot confirm this poppy's teardown, and its pass means nothing.** Diagnosed
2026-09-12 and specified for the platform in
`agentspoppy/docs/specs/certify-stackless-blindness.md`: certify's blind-sweep guard only
fires when a **CloudFormation stack** stands, and HostingPoppy creates none — it calls the
Amplify API directly. So an empty sweep reads as "nothing to clean" instead of "I may be
blind", and the run certifies without seeing anything. It is NOT the tagging index (which
covers Amplify fine) and NOT missing credentials; both were tested.

L4 is therefore listed on hand-verified evidence — twice, confirmed from outside the account
(`PLAN.md` §4). Do not cite `leaves-no-trace.cert.json`, and never let a green certify stand
in for watching the resources actually disappear. The file also carries the AWS account id it
ran against, which is why `*.cert.json` is in `.gitignore`.
