...
**v0.1.0 is built and unit-tested; it has NOT been verified against live AWS.** The frontend,
backend, manifest and tests exist and `npm run check` is the gate they pass. Every live check
— first deploy and its real timings, SPA deep links, a custom domain end to end, a certified
zero-residual teardown, the region matrix, estimate-versus-bill — is still open and listed as
**L1–L6 in `PLAN.md` §4**. Nothing has been packed, released or listed yet.
