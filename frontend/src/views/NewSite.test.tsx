...
describe("the name, before it is set in stone", () => {
  ...
  it("promises nothing it can't keep, and stays quiet when the name survives as typed", async () => {
    ...
    // The warning stays visible however short the copy gets: it is the sentence that
    // prevents regret, and the 2026-09-12 wordiness pass cut around it, never through it.
    expect(screen.getByText(/change it later/i)).toBeInTheDocument();
  });
});
