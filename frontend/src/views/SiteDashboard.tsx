    ...
    <div className="stack">
      ...
      <div className="card">
        ...
        {usage ? (
          ...
          <p style={{ marginBottom: 6 }}>
            Roughly <strong>${STORAGE_USD_PER_GB_MONTH.toFixed(3)}</strong> a month for each gigabyte your
            site takes up, plus <strong>${SERVED_USD_PER_GB.toFixed(2)}</strong> for each gigabyte visitors
            download. For a small site that's pennies. HostingPoppy can't total your real bill — it only
            asks for permission to look after the websites themselves, not your account's spending.
          </p>
        )}
        <p className="muted small" style={{ marginBottom: 0 }}>
          {AWS_PRICES_NOTE}
          ...
          <>
            ...
            <p className="muted small" style={{ margin: "10px 0 0" }}>
              {working
                ...
                : `Every push ${site.branch ? `to ${site.branch} ` : ""}goes live on its own — this is for building the newest commit again without pushing, or for starting a build that never began. Visitors keep seeing the version that's live until the new one is ready.`}
            </p>
          </>
        ) : (
          <>
